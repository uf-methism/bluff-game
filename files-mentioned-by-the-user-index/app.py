...
def build_game_payload(room):
    ...
    target_rank = VALUES[room["current_rank_index"]]
...
async def emit_room_update(room):
    await sio.emit("room_update", build_room_payload(room), room=room["id"])
...
async def start_game(sid, data):
    ...
    room["last_play"] = None
    room["turn_index"] = 0
    room["current_rank_index"] = 0
    room["last_action"] = {"type": "start", "message": "The match has started."}
    room["hands"] = {player: [] for player in players}

    for idx, card in enumerate(room["deck"]):
        room["hands"][players[idx % len(players)]].append(card)
...
async def play_cards(sid, data):
    ...
    if previous_play and len(room["hands"].get(previous_play["player"], [])) == 0:
        ...
        await sio.emit("game_over", {"winner": winner}, room=room["id"])
    ...
    claim_rank = VALUES[room["current_rank_index"]]
    room["last_play"] = {
        "player": username,
    ...
    room["current_rank_index"] = (room["current_rank_index"] + 1) % len(VALUES)
...
if __name__ == "__main__":
    print("🃏 Bluff server starting on http://0.0.0.0:8080")
